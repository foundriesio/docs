...
We leverage the tagging functionality of FoundriesFactory to simplify the development. Let's recap:
...
* ``main`` as the stable development
	...
	``main`` can be used as a reference for release candidates and marking Targets ready to production.

* ``devel`` as the experimental branch

	``devel`` is ahead of ``main`` and brings the active development.
...
* ``next`` as the LmP migration branch
	...
	Adapting a new LmP version to your Factory can be challenging, specially when there is a BSP update available. It often requires several interations until the migration is complete, so we recommend using a special branch for that.
...
- ``generic-feature`` branches
	...
	Feature specific branch similar as above, however it uses a generic name so no ``ci-scripts`` changes are required as the branch is reused for multiple features.
...
* :ref:`Local builds <ref-linux-building>`
	...
	Provides fast interations. Specially useful when debugging board support issues during LmP migrations.
...
* ``lmp-base`` :ref:`distro <ref-linux-distro>`
	...
	This provides a standard read-write rootfs, where boot files can be easily replaced for fast interations.
...
* :ref:`ug-container-preloading`
	...
	This allows to start the apps before registering to the Factory, also reducing the first update size.
...
Similar as the previous example, you do not wish that a portion of your devices automatically consume every ``devel`` build as they come from CI.
These devices then listen to the ``devel-stable`` tag and run Target ``100`` at this point.
...
This makes the set of devices to update to the Target ``110``.
...
Let's say there are two test devices: one is sitting on the lab and the other belongs to a developer.
...
The device on the lab stays on the previous Target, for example ``100``, as there is no newer Target available for the test tag.
...
.. code-block::
   ...
   $ fioctl targets list --by-tag fs-3060 -f vanessa-tuf-test
...
When talking about production workflows, you first need to define which Target to release as production.
...
Waves also allows to control how the update is delivered to your fleet.
...
.. note::
   ...
   The tagging scope for CI and production are separate: a CI Target tagged with ``production`` does not cause production devices following the ``production`` tag to update and vice-versa.
