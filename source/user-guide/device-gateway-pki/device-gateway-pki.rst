...
The following command contacts the API to generate CSRs for the server TLS certificate and the Online Device CA::
...
2. Generate a Private Key and Certificate for Root Factory CA
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
If you want to store in on an HSM device,
look at the `Fioctl Bash based PKI implementation <https://github.com/foundriesio/fioctl/blob/main/x509/bash.go>`_ for an example.
...
.. important::
    ...
    Additionally, the Common Name of your Factory Local Device CA Subject field needs to equal "fio-" plus your user ID.
...
Your Factory PKI certificates may be uploaded to the API using this Curl command::
    ...
    DEVICE_CA_CRT=$(cat device_ca_list_ca.pem | awk -v ORS='\\n' '1') \
...
You may run the following commands to verify that your device can connect to your Factory device gateway::
    ...
    # Then, use substitute the <device-gateway-ID> in the below command with your findings.
